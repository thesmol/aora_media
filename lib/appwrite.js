...
/**
 * Retrieves the latest posts from the Appwrite database.
 *
 * This function uses the Appwrite SDK to list documents in the specified video collection,
 * ordered by their creation date in descending order, and limited to the most recent 7 posts.
 * It returns an array of documents, each representing a post. The documents include details
 * such as the post's ID, title, description, and any other relevant information stored in the database.
 *
 * @returns {Promise<Array>} A promise that resolves to an array of post documents.
 ...
 * @example
 * getLatestPosts()
 *   .then(latestPosts => console.log("Latest posts:", latestPosts))
 *   .catch(error => console.error("Failed to get latest posts:", error));
 */
...
/**
 * Searches for posts in the Appwrite database based on a given query.
 *
 * This function uses the Appwrite SDK to list documents in the specified video collection
 * that match the search query provided. It performs a search operation on the 'title' field
 * of the documents in the video collection. The search query is case-insensitive and supports
 * partial matches. The function returns an array of documents that match the search criteria.
 *
 * @param {string} query - The search query string.
 * @returns {Promise<Array>} A promise that resolves to an array of post documents that match the search query.
 * Each document in the array contains details about a post, such as its ID, title, description, etc.
 * If the function fails to retrieve the posts, the promise is rejected with an error.
 *
 * @example
 * searchPosts("example")
 *   .then(posts => console.log("Search results:", posts))
 *   .catch(error => console.error("Failed to search posts:", error));
 */
